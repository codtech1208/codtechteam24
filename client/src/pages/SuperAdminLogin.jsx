...
import { Lock, Mail, Eye, EyeOff, ShieldCheck, KeyRound, CheckCircle, Send, UserCheck } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role !== 'super_admin') {
        setToast({ message: 'Access Denied: This portal is for Super Admin accounts only. Redirecting to Employee Portal...', type: 'warning' });
        setTimeout(() => {
    ...
    <div className="min-h-screen bg-white flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white rounded-3xl border border-gray-200 shadow-2xl p-8 z-10 animate-fade-in">
        ...
        <div className="text-center mb-6">
          ...
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-full text-[11px] font-extrabold uppercase tracking-wider mb-2">
            <ShieldCheck className="w-3.5 h-3.5 text-amber-600" /> Super Admin Portal
          </span>
          <h1 className="text-lg font-extrabold text-slate-900 tracking-wider uppercase mt-1">POWERED BY CODTECH TEAM</h1>
          <p className="text-[11px] font-bold uppercase tracking-widest text-brand-600 mt-0.5">Management Control Portal</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-sm shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {loading ? (
        ...
        <div className="mt-6 pt-4 border-t border-gray-100 text-center">
          <p className="text-xs text-gray-500">
            Developer / Employee?{' '}
            <Link to="/employeelogin" className="text-brand-600 font-bold hover:underline">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 animate-scale-up">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-900">Reset Super Admin Password</h3>
